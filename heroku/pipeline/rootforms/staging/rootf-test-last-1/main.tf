...
# =============================================================================
# This configuration manages the rootf-test-last-1 Heroku app, which is part of
...
# App
# -----------------------------------------------------------------------------
resource "heroku_app" "rootf_test_last_1" {
  name   = var.app_name
  region = var.region
  stack  = var.stack
...
# -----------------------------------------------------------------------------
resource "heroku_pipeline_coupling" "rootf_test_last_1" {
  app_id   = heroku_app.rootf_test_last_1.id
...
resource "heroku_formation" "rfworker" {
  ...
  depends_on = [heroku_app.rootf_test_last_1]
}
...
resource "null_resource" "github_connect" {
  triggers = {
    app_name    = heroku_app.rootf_test_last_1.name
  ...
  provisioner "local-exec" {
    command = <<-EOT
      ...
      EXISTING=$(curl -sf \
        "https://kolkrabbi.heroku.com/apps/${heroku_app.rootf_test_last_1.uuid}/github" \
      ...
      if echo "$EXISTING" | grep -q '"repo"'; then
        ...
        curl -sf -X POST \
          ...
          -d '{"app_id": "${heroku_app.rootf_test_last_1.uuid}", "repo": "${var.github_repo}"}'
      fi
    EOT
  }
